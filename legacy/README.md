...
修法在 `glitch-vn/art/live2d/refine/head_mask.py`（沿髮絲與衣服交界手描折線，線下像素一律搬出頭部層，因為用顏色分不開同色系的髮與帽T）與 `fix_head_hands.py`（指尖），用 `refine/rebuild_all.sh` 從原圖重跑整條修補管線後 Replace 重匯。
