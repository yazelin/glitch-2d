...
揮起來的仍然是**側面的手** — 手背朝外、五指斜著。`pet-plain.webp` 那隻打招呼的手是**掌心正對鏡頭**，兩者是不同的觀看角度，旋轉變不出來，這是 Live2D 也用差分圖解決的那一類。下面所有的量測都在說骨架動得對不對，沒有一條在說它讀起來像不像打招呼。正面掌心的做法寫在 `NEXT.md`，是改角色素材，要先過目。
