...
const calibrations = [];
for (const name of ['user-alignment-v0.4.0', 'user-alignment-v0.4.1']) {
...
try{
  for(const mode of ['webgl','canvas']){
    ...
    const expected={'user-alignment-v0.4.0':['arm-left','arm-right','skirt','leg-left','leg-right'],'user-alignment-v0.4.1':['arm-left','arm-right','leg-left','leg-right']};
    for(const entry of result.migrations){assert.deepEqual(entry.migration.replaced,expected[entry.name],entry.name);assert.deepEqual(entry.migration.recalibrated,['torso','face'],entry.name);assert(entry.preserved&&entry.currentArt,entry.name);}
